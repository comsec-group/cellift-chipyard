...
void vm_boot(uintptr_t test_addr_pa)
{
  /////////
  ...
  // To help Meltdown occur, put the data in l1, and then flush the TLB by making many accesses.
  ...
  // // Flush the TLB
  // for (int i = 1; i < 128; i++)
  //   l1pt[i] = ((pte_t) user_l2pt  >> PGSHIFT << PTE_PPN_SHIFT) | PTE_V;

  trapframe_t tf;
  memset(&tf, 0, sizeof(tf));

  // this sets up the user address space 
  //tf.epc = test_addr_pa - DRAM_BASE;
  tf.epc = (uint64_t)pa2uva(test_addr_pa);
  pop_tf(&tf);
}
